...
const ScNumberInput = styled(NumberInput)`
  padding: 0 0.25rem;
`

class AutosizeNumberInput extends Component {
  static propTypes = {
  ...
  copyInputStyles() {
    ...
    const inputStyles =
      this.input && window.getComputedStyle(ReactDOM.findDOMNode(this.input)) // eslint-disable-line react/no-find-dom-node
    if (!inputStyles) {
      return
    }
  ...
  updateInputWidth() {
    ...
    const extraWidth =
      this.props.type === 'number' && this.props.extraWidth === undefined
        ? 16
        : Number(this.props.extraWidth) || 0
  ...
  render() {
    ...
    return (
      <ScWrapper>
        ...
        <ScNumberInput {...inputProps} ref={this.inputRef} />
...
export default withTheme(AutosizeNumberInput)
